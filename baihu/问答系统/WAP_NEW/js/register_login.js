...
var register_login = {
    ...
    reg_register:function(){
        var register_phone =$.trim($('#phone_reg').val()),

            resister_pass = $.trim($('#pass_reg').val()),

            register_pass_again = $.trim($('#pass_reg_again').val());
        if(register_phone==""){
            layer.open({
                skin:'msg',
                content:"请输入手机号",
            ...
            return false;
        }else if(!this.punElem.reg.test(register_phone)){
            layer.open({
                skin:'msg',
                content:"请输入正确的手机号",
                time:1.5
