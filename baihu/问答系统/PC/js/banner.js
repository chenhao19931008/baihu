...
function banner(ajaxurl,time){
	...
	function bindDate(){
		...
		$.each(jsonData, function(index,item) {
			...
			str+="<li class='banner-item' data-name="+index+"><div class='banner-item-main'><a href='#'><img src='"+item["src"]+"' alt=''/></a></div></li>";
		...
		if(jsonData.length>1){//如果页面中药执行动画的元素个数大于1才开始轮播
			//
			var tag = $("<li class='banner-item' data-name='0'><div class='banner-item-main'><a href='#'><img src='"+jsonData[0]["src"]+"' alt=''/></a></div></li>");
...
$(function(){
	...
	$('.lv_banner_wrap').banner('json/jsonData.txt',5000);

	var lis = $('.lv_banner_swiper li');
    ...
    lis.each(function(index,el){
        ...
        if(dataName==0){
				...
				$(this).css('background',"#6ac7f5");
		}else if(dataName==1){
            $(this).css({
            	'background':"url(./img/banner/bg3.png) center center no-repeat",
            ...
            $(this).css({
            	'background':"url(./img/banner/bg2.png) center center no-repeat",
				'background-size':'cover'
            });
		}
